...
class ReviewPreprocessor:
    ...
    def remove_duplicates(self):
        """Remove duplicate reviews per bank based on review_text"""
        print("\n[5/8] Removing duplicate reviews per bank...")

        before_count = len(self.df)

        # Drop duplicates based on both 'review_text' and 'bank_name'
        self.df = self.df.drop_duplicates(subset=['review_text', 'bank_name'], keep='first')

        removed = before_count - len(self.df)
        print(f"Removed {removed} duplicate reviews (per bank)")

        # Record statistics in self.stats
